...
import { parseInterval } from "simple-signal";
...
export class BroadcastRunner {
  ...
  register(definition: BroadcastDefinition): this {
    if (this.registry.has(definition.name)) {
      console.warn(
        `[simple-broadcast] Duplicate broadcast name "${definition.name}" — overwriting.`,
      );
    }
  ...
  async start(): Promise<void> {
    ...
    const shutdown = () => {
      console.log("[simple-broadcast] Received shutdown signal, stopping...");
      this.stop({ graceful: true, timeoutMs: 10_000 }).catch((err) => {
        console.error("[simple-broadcast] Error during shutdown:", err);
      });
    };
    ...
    while (this.running) {
      try {
        ...
        console.error("[simple-broadcast] tick() failed:", err);
      }
  ...
  private emit<K extends keyof BroadcastSubscriber>(
    ...
    for (const sub of this.subscribers) {
      try {
        ...
        console.error(`[simple-broadcast] Subscriber error in ${String(event)}:`, err);
      }
    }
  }
  ...
  private async discover(dir: string): Promise<void> {
    ...
    try {
      ...
      console.error(`[simple-broadcast] Cannot read broadcastsDir: ${dir}`);
      return;
    }
    ...
    for (const filePath of files) {
      try {
        ...
        console.warn(`[simple-broadcast] Skipping ${filePath} — failed to import (if .ts, ensure a TypeScript loader like tsx is active):`, err);
      }
    }
  }
