...
export function configureBroadcast(options: { adapter: BroadcastQueueAdapter }): void {
  if (_configured) {
    console.warn(
      "[simple-broadcast] configureBroadcast() called multiple times. The previous adapter will be replaced.",
    );
  }
...
export function getBroadcastAdapter(): BroadcastQueueAdapter {
  if (!_configured && !_warnedUnconfigured) {
    ...
    console.warn(
      "[simple-broadcast] No adapter configured — using default BroadcastMemoryAdapter. " +
