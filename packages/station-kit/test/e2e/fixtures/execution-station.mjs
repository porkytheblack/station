...
if (options.primitive === 'sandbox') execution.sandbox = new HostSandboxAdapter({
  rootDir: join(options.cwd, 'workspaces'), maxConcurrent: 2, maxTimeoutMs: 60_000,
});
if (options.primitive === 'bun' || options.primitive === 'playwright') {
  execution.browser = new BrowserSessionManager(options.primitive === 'bun' ? new BunBrowserAdapter() : new PlaywrightBrowserAdapter(), 2);
}
