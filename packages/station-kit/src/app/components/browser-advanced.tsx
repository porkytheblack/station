...
export function BrowserAdvanced({ owner, sessionId, admit, reachable, features, onBusy, profile, onProfile }: { owner: string; sessionId: string; admit: boolean; reachable: boolean; features: Record<string, boolean>; onBusy: (value: boolean) => void; profile: string; onProfile: (value: string) => void }) {
  const [pages, setPages] = useState<BrowserPage[]>([]), [profiles, setProfiles] = useState<BrowserProfile[]>([]);
  const [url, setUrl] = useState(""), [op, setOp] = useState("fill"), [selector, setSelector] = useState(""), [value, setValue] = useState(""), [checked, setChecked] = useState(true);
  ...
  useEffect(() => {
    if (!reachable || !features.profiles) return;
    let cancelled = false;
  ...
  }, [rpc, reachable, features.profiles, sessionId]);
  ...
  const perform = () => {
    const command: BrowserCommand = op === "fill" ? { op, selector, value } : op === "select" ? { op, selector, values: value.split(",") } : op === "check" ? { op, selector, checked } : op === "hover" ? { op, selector } : op === "waitFor" ? { op, selector, state: "visible" } : op === "scroll" ? { op, x: 0, y: Number(value) } : op === "download" ? { op, selector } : { op: op as "content" | "back" | "forward" | "reload" };
    void run(command);
  };
  return <section className="station-card execution-card" aria-label="Advanced browser controls"><h2>Pages and browser tools</h2><ExecutionAlert error={error} />
    {features.profiles && <><label className="execution-field"><span>Saved profiles</span><select className="input-text" aria-label="Saved browser profile" value={profile} disabled={busy} onChange={e => onProfile(e.target.value)}><option value="">Ephemeral session</option>{profiles.map(p => <option key={p.id} value={p.id}>{p.id}{p.inUse ? " · in use" : ""}</option>)}</select></label><button className="btn btn--danger" disabled={!admit || busy || !profile || profiles.find(p => p.id === profile)?.inUse} onClick={() => { if (!window.confirm(`Delete saved profile ${profile}?`)) return; setBusy(true); void rpc({ method: "profileDelete", id: profile }).then(() => { setProfiles(old => old.filter(p => p.id !== profile)); onProfile(""); }).catch(e => setError(executionError(e))).finally(() => setBusy(false)); }}>Delete profile</button></>}
    ...
    {sessionId && <><form onSubmit={e => { e.preventDefault(); if (admit && !busy) perform(); }}><label className="execution-field"><span>Page operation</span><select className="input-text" aria-label="Page operation" value={op} onChange={e => setOp(e.target.value)}>{["fill", "select", "check", "hover", "waitFor", "scroll", "content", "back", "forward", "reload", ...(features.downloads ? ["download"] : [])].map(name => <option key={name} value={name}>{name}</option>)}</select></label>
      {["fill", "select", "check", "hover", "waitFor", "download"].includes(op) && <label className="execution-field"><span>Element selector</span><input className="input-text" aria-label="Element selector" value={selector} onChange={e => setSelector(e.target.value)} /></label>}
      {["fill", "select", "scroll"].includes(op) && <label className="execution-field"><span>{op === "select" ? "Values separated by commas" : op === "scroll" ? "Vertical pixels" : "Value"}</span><input className="input-text" aria-label="Page operation value" value={value} onChange={e => setValue(e.target.value)} /></label>}
      ...
      {features.uploads && <label className="execution-field"><span>Upload to the element selector (up to 4 MiB)</span><input type="file" aria-label="Browser upload file" disabled={!admit || busy || !selector} onChange={e => { const file = e.target.files?.[0]; if (!file) return; if (file.size > 4 * 1024 * 1024) { setError("Upload exceeds 4 MiB."); return; } const reader = new FileReader(); reader.onerror = () => setError("Could not read file."); reader.onload = () => void run({ op: "upload", selector, files: [{ name: file.name, mimeType: file.type || "application/octet-stream", base64: String(reader.result).split(",")[1] }] }); reader.readAsDataURL(file); e.target.value = ""; }} /></label>}
