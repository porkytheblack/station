...
export function v1TriggerRoutes(deps: V1TriggerDeps) {
  ...
  app.post("/runs/:id/rerun", async (c) => {
    ...
    await deps.signalAdapter.addRun({
      ...
      requiredStationId: run.requiredStationId,
      kind: "trigger",
      input: run.input,
      status: "pending",
      attempts: 0,
  ...
  app.post("/broadcast-runs/:id/rerun", async (c) => {
    ...
    try {
      ...
      const newId = await deps.broadcastRunner.trigger(run.broadcastName, input, { requiredStationId: run.definitionSnapshot ? JSON.parse(run.definitionSnapshot).requiredStationId : undefined });
      return c.json(
