...
const nextServer = resolve(kitRoot, '../station-dashboard/bin/station-dashboard.mjs');
...
test('real dashboard controls private sandbox and Bun/Playwright workers; custom installs survive worker restart', { timeout: 420_000 }, async (t) => {
  assert.ok(existsSync(nextServer), 'Build station-daemon and station-dashboard before running E2E.');
  ...
  const hq = await startWorker('hq');
  dashboard = processHandle(process.execPath, [nextServer], { cwd: kitRoot, env: { ...process.env, STATION_DAEMON_URL: `http://127.0.0.1:${hq.port}`, PORT: String(nextPort), HOSTNAME: '127.0.0.1' }, stdio: ['ignore', 'pipe', 'pipe'] });
  const base = `http://127.0.0.1:${nextPort}`;
  await until(async () => { try { return (await fetch(`${base}/sandboxes`)).status === 200; } catch { return false; } }, 'Standalone dashboard startup');
