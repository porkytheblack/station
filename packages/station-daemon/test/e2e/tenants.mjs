...
test('public tenant keys control real isolated containers through Headquarters without cross-tenant access', { skip: !executable || !browserImage, timeout: 300_000 }, async t => {
  ...
  const fixture = resolve('packages/station-daemon/test/e2e/fixtures/custom-tool');
  ...
  assert.throws(() => new ContainerSandboxAdapter({ rootDir: join(root, 'a', 'sandbox'), tenantId: 'b', executable, engine, seccompProfile: process.env.STATION_CONTAINER_SECCOMP, image: process.env.STATION_CONTAINER_IMAGE ?? 'docker.io/library/node:22-bookworm-slim' }), /tenant|bound|ownership/i);
  assert.throws(() => new ContainerBrowserAdapter({ rootDir: join(root, 'a', 'browser'), tenantId: 'b', executable, engine, seccompProfile: process.env.STATION_CONTAINER_SECCOMP, image: browserImage }), /tenant|bound|ownership/i);
  const recovered = new ContainerSandboxAdapter({ rootDir: join(root, 'a', 'sandbox'), tenantId: 'a', executable, engine, seccompProfile: process.env.STATION_CONTAINER_SECCOMP, image: process.env.STATION_CONTAINER_IMAGE ?? 'docker.io/library/node:22-bookworm-slim', network: 'none', enablePty: false });
