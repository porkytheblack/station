...
test('agent toolset and image bridge use authenticated Headquarters and a real Playwright worker', { timeout: 60_000 }, async t => {
  ...
  const worker = await createStation(resolveConfig({ role: 'station', name: 'browser-tools', host: '127.0.0.1', port: workerPort,  runRunners: false, network: { id: 'browser-agent', stationId: 'browser', adapter: network, endpoint: `http://127.0.0.1:${workerPort}` }, execution: { token, browser: manager } }), join(root, 'worker'));
  stations.push(worker); await worker.start();
  const hqPort = await freePort();
  const hq = await createStation(resolveConfig({ role: 'headquarters', host: '127.0.0.1', port: hqPort,  runRunners: false, network: { id: 'browser-agent', stationId: 'hq', adapter: network }, auth: { username: 'operator', password: randomBytes(20).toString('hex'), keyStorage: storage }, execution: { token } }), join(root, 'hq'));
