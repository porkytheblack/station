...
test("station shutdown closes browser actions before waiting for HTTP connections", { timeout: 10_000 }, async (t) => {
  ...
  const station = await createStation(resolveConfig({ port, host: "127.0.0.1", runRunners: false,  network: { stationId: "browser" }, auth: { username: "operator", password: "test-password" }, execution: { token, browser } }), root);
  let stopped = false;
...
test('Headquarters preserves worker busy conflicts and sanitizes uncertain or malformed failures without retry', async t => {
  ...
  worker.post('/internal/execution/browser', c => {
    ...
    if (mode === 'challenge_required') return c.json({ error: mode, message: token }, 409);
    if (mode === 'rate_limited' || mode === 'provider_capacity') return c.json({ error: mode, message: token }, 429);
    if (mode === 'provider_auth' || mode === 'provider_unavailable') return c.json({ error: mode, message: token }, 503);
  ...
  for (const [scenario, status] of [['challenge_required', 409], ['rate_limited', 429], ['provider_capacity', 429], ['provider_auth', 503], ['provider_unavailable', 503]] as const) {
    mode = scenario; const response = await call(); assert.equal(response.status, status);
    const body = await response.json(); assert.equal(body.error, scenario); assert.ok(!JSON.stringify(body).includes(token));
    assert.equal(body.outcome, status >= 500 ? 'unknown' : undefined);
  }
