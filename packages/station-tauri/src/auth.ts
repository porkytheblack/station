 ...
 */
export function getOrCreateApiKey(keyStore: KeyStore, dataDir: string): string {
  ...
  if (existsSync(keyFilePath)) {
    ...
    if (savedKey) {
      const record = keyStore.verify(savedKey);
      if (record) {
        return savedKey;
      }
    }
  ...
  const { key } = keyStore.create("tauri-desktop", ["trigger", "read", "cancel", "admin"]);
