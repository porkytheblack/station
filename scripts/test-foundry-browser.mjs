...
import { createStation } from '../packages/station-daemon/src/server/index.ts';
import { resolveConfig } from '../packages/station-daemon/src/config/schema.ts';
import { KeyStore, MemoryKeyStorage } from '../packages/station-daemon/src/server/auth/keys.ts';
...
try {
  ...
  const worker = await createStation(resolveConfig({ role: 'station', name: 'foundry-browser', host: '127.0.0.1', port: workerPort, runRunners: false, network: { id: 'foundry-e2e', stationId: 'browser', adapter: network, endpoint: `http://127.0.0.1:${workerPort}` }, execution: { token, browser: manager } }), join(root, 'worker'));
  stations.push(worker); await worker.start();
  const hqPort = await freePort();
  const hq = await createStation(resolveConfig({ role: 'headquarters', host: '127.0.0.1', port: hqPort, runRunners: false, network: { id: 'foundry-e2e', stationId: 'hq', adapter: network }, auth: { username: 'operator', password: randomBytes(20).toString('hex'), keyStorage }, execution: { token } }), join(root, 'hq'));
