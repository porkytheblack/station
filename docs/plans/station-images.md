...
The workspace now separates `station-daemon`, `station-runtime-cli`, `station-dashboard` and `station-client`, with coordinated 3.0.0 versions and no maintained `station-kit` package. `station-images` provides immutable manifests/blobs, native and bundled-JavaScript validation, finite signal/planner and long-lived beacon protocols, environment grants, dependency validation and trusted-local/Docker backend interfaces. The daemon exposes operator-admin registry publish/resolve/tag/pull/install/run APIs, persists image activation roots, integrates digest-qualified definitions with existing runners and periodically imports compatible Headquarters catalog images on configured workers. Broadcast plans are persisted before child execution; beacon dependency triggers go through a scoped supervisor broker. The CLI has contexts, detached local management, API/execution wrappers, registry commands and a read-only TUI. Packed isolated installation and real host-process execution paths have been tested. Explicit worker pins are supported for signals and planned broadcasts; pinned beacons are created through their owning worker. Docker execution has a separate expiry-reaper command backed by durable invocation journals and ownership labels.

The complete public multi-tenant deployment product described below is not implemented. Customer-scoped registry access, richer deployment generations and rollback/GC, on-demand preparation reservations, full interactive TUI/image dashboard flows, automatic enrollment, VM execution and production isolation/failover acceptance remain separate work. Headquarters distribution currently uses eager periodic catalog synchronization. Do not interpret later examples such as `image build`, `deployment rollback`, network enrollment or a VM terminal as available commands. Native cross-compilation and application bundling remain author/operator responsibilities; the CLI publishes explicit prebuilt artifacts.

Five real Docker tests passed on Docker Desktop 27.5.1’s Linux arm64 engine: isolated JavaScript/native invocation, applied isolation settings, timeout cleanup and independent reaping of an expired running container. See the image guide for the pinned runtime, seccomp policy, reproduction steps and limits of that evidence. Acceptance on the intended production Linux configuration and the final release dry run remain release gates; unit tests alone do not establish those guarantees. npm publication and StationKit deprecation remain explicit release operations.
