...
function renderIndex() {
  ...
  return `# ${site.name}\n\n> ${site.summary}\n\nStation ${packageManifest.version} is a modular TypeScript framework. Station 3.0 separates station-daemon (headless runners/API, stationd), station-runtime-cli (station command), station-client (transport), and station-dashboard (independent UI). station-kit is retired without compatibility exports. Start the daemon with stationd; start station-dashboard separately with STATION_DAEMON_URL, PORT and STATION_DASHBOARD_HOST. Clients can connect to remote daemons and closing them does not stop work. station-images packages independently compiled native or bundled JavaScript signals, broadcast planners and beacons under station.process/v1. Operator-admin registries support publish/install/run, granted environment values, compatible Headquarters catalog synchronization and immutable worker pins. Registry metadata and blobs use separate pluggable adapters with shared validation; file/memory implementations are included. Custom registry.storage backends use verified local execution caches and keep provider credentials in the daemon; PostgreSQL/S3 registry drivers are not bundled. Docker image execution requires reviewed runtime policy and an independent station-image-reaper; tenant-scoped registry grants and a fixed dedicated-worker execution gateway are implemented with real Docker fixture coverage. Enrollment admission/revocation, reconnectable bounded-replay TUI streams, mixed native planner grants, scoped invocation artifact brokering, audited environment overrides and explicit beacon rollout have executable acceptance coverage. Invocation media uses operator-granted local storage without automatic cross-worker transfer or a general media HTTP API. Intended Linux production-host validation was explicitly deferred. The 20-package release dry run and isolated packed installs passed; npm publish permissions remain unverified. Check the acceptance map for the exact tested boundaries. The full deployment platform remains incomplete. For browser-local work, use the experimental station-browser runtime with explicit registries and IndexedDB; it has been available since Station 2.3.0. Browser workers use cooperative execution and cannot guarantee continuous work after a PWA closes. In the Node runtime, signals are isolated background jobs, broadcasts are DAG workflows, beacons are supervised long-running processes, schedules provide interval and timezone-aware cron execution, and Station Networks coordinate work across a Headquarters and worker stations. The experimental station-sandbox and station-browser-use packages add separate persistent workspaces and agent-controlled browser sessions, routed to their owning workers through Headquarters. Host adapters are for trusted workloads. Public tenants require scoped keys, dedicated private workers, container backends and independently enforced network/storage controls. Browser agent tools provide workflow-scoped sessions, structured DOM/ARIA observations and native image results; the Foundry example demonstrates the model image bridge. Profiles, files and configured recordings can persist, but live processes are not restored. Local browser/protocol tests do not require a model key; the paid real-model test is opt-in. These are execution primitives, not a complete multi-tenant hosting platform. Node remains the default child runtime; Bun signal/beacon execution is opt-in through ProcessRuntime. Prefer the Markdown links below when supplying documentation to an agent.\n\n${groups.join("\n\n")}\n\n## Optional\n\n- [Complete documentation](${site.origin}/llms-full.txt): All documentation pages combined into one Markdown document for larger context windows.\n- [Human documentation](${site.origin}/docs/getting-started): Rendered documentation site.\n- [Remote Browser Use guide](https://github.com/porkytheblack/station/blob/main/packages/station-browser-use/REMOTE.md): Browserbase/Steel, persistent WhatsApp/TikTok/Instagram account workflows, upload/download limits, proxies, profile grants, pacing, human takeover and lifecycle recovery.\n- [Detailed Sandbox guide](https://github.com/porkytheblack/station/blob/main/packages/station-sandbox/GUIDE.md): Custom tool installation, Git workflows, files, terminals, services, tenant RPCs and backend-specific recovery.\n- [GitHub repository](https://github.com/porkytheblack/station): Source, package implementations, and runnable examples.\n`;
}
