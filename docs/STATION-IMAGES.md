...
Both destination and `fromEnv` source keys must be in the operator's `allowedEnv` list. Literal `value` bindings are persisted non-secret configuration; never put credentials there. `fromEnv` persists only the key reference and resolves the value from the worker's scoped environment store for each attempt. Each generation receives distinct registered identities, so later bindings do not rewrite existing definitions. The reference is immutable; the secret value is **not** revision-pinned. Changing the source store can affect retries or later attempts using the same reference. A deployment can explicitly authorize per-invocation binding keys with `invocationEnv`. Supplying `environment` when invoking an alias creates a retained derived generation, or reuses the existing generation for the same source generation and identical effective bindings. Every invocation records an `invoke` history entry; it does not change the active alias pointer. Both ordinary generation grants and that invocation allowlist apply. This records binding references and non-secret values, never resolved secrets. Secret-store revisions are not versioned: retries resolve current values. Existing live beacons retain their process environment until an explicit restart or the rollout operation below.

Each deployment permits 128 ordinary generations and 128 distinct derived invocation configurations. Repeated equivalent overrides reuse their immutable identity. Reaching the invocation limit rejects new distinct configurations without preventing stage, activation, rollback or drain; legacy snapshots containing 256 generations retain their identities and control operations. No queued execution identities are evicted. Audit history retains at most 512 entries per deployment and 4,096 across the namespace; rollback eligibility is retained separately from this window. New generations and rollout intents preserve 512 KiB of the 8 MiB deployment snapshot limit for control operations. Control mutations may trim retained audit further when processing an older full snapshot. This is bounded retention, not automatic image garbage collection. Runtime activation snapshots independently reject writes over their 1 MiB read limit before replacing the previous valid state.
...
Enrollment is separate from saving a CLI context or supplying shared queue credentials. The authority issues short-lived single-use invitations bound to a fixed network and worker ID. Redeeming an invitation returns a worker credential; the durable store retains only hashes, serializes independent writers and persists revocation. Explicit operator re-enrollment rotates the credential generation and invalidates the earlier credential. The file authority publishes fully initialized lock directories atomically and checks host/process birth identity (including Linux boot identity) before retiring a dead owner. Recovery retains a nonempty tombstone per owner nonce so concurrent recovery cannot displace a newer writer. Live owners remain protected. Missing, legacy or unverifiable ownership fails closed with the lock path; verify all previous authorities stopped before manually moving such a lock aside. Retired tombstones and abandoned pre-publication temporary directories can be removed during controlled maintenance while authorities are stopped. This local filesystem protocol is not distributed authority consensus.

Operator endpoints are `POST /api/v1/network/enrollments`, `GET /api/v1/network/members` and `DELETE /api/v1/network/members/:id`. Invitation redemption uses `POST /api/v1/network/join`; worker-credential checks and voluntary revocation use `POST /api/v1/network/admission` and `/network/leave`. Invitations expire after five minutes by default (at most fifteen). Worker admission uses a fixed HTTPS authority origin, bounded response/time limits and a fresh check rather than a cached authorization lease. Initial admission and new claims require successful fresh checks. Existing admitted work may retain heartbeat/lease renewal during transport failures, timeouts, HTTP 408/429 or 5xx responses for a bounded outage grace: `network.enrollment.renewalGraceMs` defaults to 30,000 and accepts 0–120,000 milliseconds. Explicit rejection or revoked/mismatched identity clears that grace immediately; an outage cannot resurrect a revoked worker. `network.enrollment.timeoutMs` defaults to 2,000 and accepts 100–4,000 milliseconds, below the runner's 5-second admission deadline. Once grace expires, renewals fail closed and supervised work is fenced. A real BeaconRunner test verifies a child survives both 503 and timeout within grace and is reaped after explicit rejection. A real daemon test revokes an active worker and verifies that it stops execution, cannot claim new work and cannot restore membership merely by heartbeating. Enrollment does not distribute database credentials or revoke a malicious process's independently acquired database access.
...
A deleted or identity-changed source marks that rollout `failedAt` with an error, allowing other deployments to reconcile. Cancel a pending or failed intent with `POST /api/v1/registry/deployments/:id/rollouts/:rolloutId/cancel` and `{expectedRevision}`, or:

```sh
station deployments rollout-cancel DEPLOYMENT_ID --json '{"rolloutId":"rotate-worker-a-1","expectedRevision":8}'
```

Cancellation records `cancelledAt`; reconciliation checks current intent before each subsequent step. It does not undo an already-dispatched stop, revive the source, or remove a replacement that already exists. Inspect those instances and explicitly manage their lifecycle. Completed rollouts cannot be cancelled.
...
| `POST /registry/deployments/:id/rollouts/:rolloutId/cancel` | `{expectedRevision}` | Cancel future reconciliation; existing instance effects remain |
