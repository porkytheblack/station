...
For local preparation without provider credentials or inference, append `--protocol-only`. That explicit mode substitutes only the model import in the temporary copy, then verifies public Foundry discovery, instance/conversation execution and mounting of the real browser tool schemas. Its report contains `realModel:false`; it does not prove model-driven browsing. Separately, `node --import tsx --test packages/station-daemon/test/e2e/browser-agent-tools.mjs` exercises the real authenticated client/Headquarters/Playwright path and screenshot bridge without inference. Run the bridge's protocol unit tests with `node --test examples/19-foundry-browser/browser-bridge.test.mjs`.
